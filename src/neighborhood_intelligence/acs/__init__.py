...
class CensusAcsClient:
    ...
    def fetch_state_tracts(self, year: int, state_fips: str) -> tuple[list[dict[str, str]], str, bytes]:
        ...
        if self.settings.census_api_key:
            params.append(("key", self.settings.census_api_key))
        ...
        # The request key is intentionally excluded from lineage: it is a secret,
        # while the remaining query parameters are sufficient for reproducibility.
        lineage_params = [(name, value) for name, value in params if name != "key"]
